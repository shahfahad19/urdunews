...
export default class Pakistan extends Component {
    _isMounted = false;
    cities = [
        "karachi",
        "lahore",
    ...
    async componentDidMount() {
        ...
        let data = await this.sendRequest(
            "https://api.codetabs.com/v1/proxy?quest=https://dailypakistan.com.pk/E-Paper/" +
                this.city
        );
        this.getImages(data);
        if (this._isMounted) {
        ...
        return (
            <div>
                ...
                {this.state.images.map((link, index) => {
                    return <Image src={link} key={index} page={index} />;
                })}
            </div>
        );
    }
